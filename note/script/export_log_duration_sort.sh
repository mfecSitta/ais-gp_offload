...
# Function sort
sort_data_column_5() {
    local input_file_name_scr=$1
    local input_file_name_tgt=$2

    echo "--------------------------------------------------"
    echo "Processing Sort: ${input_file_name_scr}"

    # 1. นับจำนวนบรรทัดไฟล์ต้นทาง (Source)
    local count_src=$(wc -l < "${input_file_name_scr}")
    echo "   - Source lines: ${count_src}"

    echo "   - Command: sort -t ',' -k5,5r ${input_file_name_scr} > ${input_file_name_tgt}"
    
    # 2. สั่ง Sort
    sort -t ',' -k5,5r "${input_file_name_scr}" > "${input_file_name_tgt}"
    
    # ตรวจสอบ Exit Code ของคำสั่ง Sort
    if [ $? -ne 0 ]; then
        echo "❌ Error: Sort command failed. (${input_file_name_scr})"
        exit 1
    fi

    # 3. นับจำนวนบรรทัดไฟล์ปลายทาง (Target)
    local count_tgt=$(wc -l < "${input_file_name_tgt}")
    echo "   - Target lines: ${count_tgt}"

    # 4. เปรียบเทียบ (Compare)
    if [ "$count_src" -ne "$count_tgt" ]; then
        echo "❌ Error: Line count mismatch!"
        echo "   Source ($count_src) != Target ($count_tgt)"
        # ลบไฟล์ปลายทางทิ้งเพื่อความปลอดภัย เพราะข้อมูลไม่ครบ
        # rm "${input_file_name_tgt}" 
        exit 1
    else
        echo "✅ Verification: PASS (Counts match)"
    fi
    
    echo ""
    echo "sort file done. -> ${input_file_name_tgt}"
    echo "--------------------------------------------------"
}
...
FULL_OUTPUT_SORT_PATH="${path_log_output}sort_${file_name}"
...
if [ "$GRAND_TOTAL_EXPECTED_LINES" -eq "$ACTUAL_OUTPUT_LINES" ]; then
    ...
    sort_data_column_5 "${FULL_OUTPUT_PATH}" "${FULL_OUTPUT_SORT_PATH}"
else
...
echo "Sorted Output File: ${FULL_OUTPUT_SORT_PATH}"
